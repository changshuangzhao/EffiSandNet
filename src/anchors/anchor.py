...
def anchor_targets_bbox(
    ...
    for annotations in annotations_group:
        ...
        assert ('properties' in annotations), 'Annotations should contain properties.'
    ...
    for index, (image, annotations) in enumerate(zip(image_group, annotations_group)):
        if annotations['bboxes'].shape[0]:
            ...
            properties_batch[index, ignore_indices, -1] = -1
            properties_batch[index, positive_indices, -1] = 1

            regression_batch[index, ignore_indices, -1] = -1
            regression_batch[index, positive_indices, -1] = 1

            # compute target class labels one-hot
            labels_batch[index, positive_indices, annotations['labels'][argmax_overlaps_inds[positive_indices]].astype(int)] = 1
            # compute target class properties one-hot
            properties_batch[index, positive_indices, annotations['properties'][argmax_overlaps_inds[positive_indices]].astype(int)] = 1
...
def compute_gt_annotations(
    ...
    """
    # (N, K)
    overlaps = compute_iou(anchors.astype(np.float64), annotations.astype(np.float64))
    # (N, )
