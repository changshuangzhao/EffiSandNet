...
def regression(fpn_features, w_head, d_head, num_anchors):
    options = {
        'kernel_size': 3,
        'strides': 1,
        'padding': 'same',
    ...
    box_convs = [layers.SeparableConv2D(filters=w_head, bias_initializer='zeros', name=f'box_net/box-{i}', **options)
                 for i in range(d_head)]
    box_head_conv = layers.SeparableConv2D(filters=4 * num_anchors,
                                           # bias_initializer=PriorProbability(probability=0.01),
                                           bias_initializer='zeros',
                                           name=f'box_net/box-predict', **options)
    box_bns = [[layers.BatchNormalization(momentum=MOMENTUM, epsilon=EPSILON, name=f'box_net/box-{i}-bn-{j}') for j in
                range(3, 8)] for i in range(d_head)]
...
def classification(fpn_features, w_head, d_head, num_anchors, num_classes):
    options = {
        'kernel_size': 3,
        'strides': 1,
        'padding': 'same',
    ...
    cls_convs = [
        layers.SeparableConv2D(filters=w_head, bias_initializer='zeros', name=f'class_net/class-{i}', **options) for i
        in range(d_head)]
    cls_head_conv = layers.SeparableConv2D(filters=num_classes * num_anchors,
                                           bias_initializer=PriorProbability(probability=0.001),
                                           name='class_net/class-predict', **options)
    cls_bns = [
        [layers.BatchNormalization(momentum=MOMENTUM, epsilon=EPSILON, name=f'class_net/class-{i}-bn-{j}') for j in
         range(3, 8)] for i in range(d_head)]
...
def properties(fpn_features, w_head, d_head, num_anchors, num_properties):
    options = {
        'kernel_size': 3,
        'strides': 1,
        'padding': 'same',
    ...
    for i, feature in enumerate(fpn_features):
        ...
        pro.append(feature)
    pro = layers.Concatenate(axis=1, name='pro')(pro)
    return pro