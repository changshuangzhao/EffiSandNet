...
from anchor import anchors_for_shape, anchor_targets_bbox, AnchorParameters
...
class Generator(keras.utils.Sequence):
    ...
    def __init__(
        ...
        self.mosaic_effect = mosaic_effect
    ...
    def num_properties(self):
        ...
        """
        raise NotImplementedError('num_properties method not implemented')
    ...
    def load_annotations_group(self, group):
        ...
        for annotations in annotations_group:
            ...
            assert ('properties' in annotations), '\'load_annotations\' should return a list of dictionaries that contain \'labels\' and \'bboxes\'.'
    ...
    def random_mosaic_group(self, image_group, annotations_group):
        if self.mosaic_effect is None:
            return image_group, annotations_group
        assert (len(image_group) == len(annotations_group))
        assert (len(image_group) % 4 == 0)
        new_image_group, new_annotations_group = [], []
        for i in range(len(image_group)//4):
            tx, ty = self.mosaic_effect(image_group[i*4:4*i+4], annotations_group[i*4:i*4+4], self.image_size)
            new_image_group.append(tx)
            new_annotations_group.append(ty)
        return new_image_group, new_annotations_group
    ...
    def group_images(self):
        ...
        if self.mosaic_effect is None:
            ...
            i = 0
            self.groups = []
            while i < len(order) - self.batch_size:
                if random.random() < self.mosaic_ratio and i + self.batch_size * 4 < len(order):
                    self.groups.append([order[x % len(order)] for x in range(i, i + self.batch_size * 4)])
                    i += self.batch_size * 4
                else:
                    self.groups.append([order[x % len(order)] for x in range(i, i + self.batch_size)])
                    i += self.batch_size
    ...
    def compute_inputs(self, image_group, annotations_group):
        ...
        # print(batch_images.shape)  # (1, 896, 896, 3)
        return [batch_images]
    ...
    def compute_inputs_targets(self, group, debug=False):
        ...
        """
        # 判断当前的group是否应用mosaic
        mosaic = len(group) == 4 * self.batch_size
        ...
        if mosaic:
            image_group, annotations_group = self.random_mosaic_group(image_group, annotations_group)
        else:
