...
sys.path.append(os.path.join(os.path.dirname(__file__), '../augmentor'))
from color import VisualEffect
...
def params():
    ...
    parser.add_argument('--score_threshold', '-s', default=0.5, type=float,
...
def main():
    ...
    model, prediction_model = efficientdet_sand(num_anchors, num_classes, num_properties, cfg.w_bifpn, cfg.d_bifpn, cfg.d_head, args.score_threshold, args.nms_threshold)
    ...
    if args.gpus and len(args.gpus.split(',')) > 1:
        model = keras.utils.multi_gpu_model(model, gpus=list(map(int, args.gpus.split(','))))

    model.compile(optimizer=Adam(lr=1e-4), loss={'regression_sand': smooth_l1(), 'classification_sand': focal(), 'pro_sand': bce()})
